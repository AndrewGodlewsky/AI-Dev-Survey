...
- **Safeguard item** — A multi-select asking which checks a Respondent accepts **in place of reading AI's output themselves** (test suite, schema check, another reviewer, easy rollback, small blast radius, security scan — plus "I'd read it myself"). Asked in the Current-state section, in the forward personal block, and once in the Team section under Verification (v1's three passes, the team one re-homed as a standalone norm). A *Respondent-level* attribute, not a Dimension item — the same safeguards answer Trust and Autonomy at once — and never scored. A Checklist item: deliberately **not** phrased as a condition, because a threshold question is banned by the Framing rule. Text in `survey/safeguard-items.md`.
