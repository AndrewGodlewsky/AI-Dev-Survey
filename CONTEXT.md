...
- **Coverage** — Not a single score but a *profile*: an **Involvement level** per **Development area** (see below), asked twice — the frozen *today pass* (Current level) and the *Direction pass* (Direction level). For the Respondent only; there is no team grid in v2.
- **Direction pass** — The second Coverage grid: the same 19 Development areas on the same Involvement level scale, answered once as *where do you want AI* — a Direction item in grid form. No horizon appears in it. Text in `survey/coverage-grid.md`. *Avoid:* six-month pass, Future-Personal pass, "me in 6 months".
- **Direction level** — A Respondent's Involvement level on a Development area in the Direction pass; the today pass's value is the **Current level**. Appetite, never a forecast.
- **Coverage Gap** — Direction level minus Current level, per Development area, in signed levels; for the Team, the median of within-person moves, never averaged across areas. Read as appetite minus practice. Exists only where both passes are numeric; an N/A on either side is a work-distribution signal, counted and flagged, never a Gap. *Avoid:* Coverage Stance gap (dead with the team pass).
...
- **Slice** — One way of cutting the scored results: by Dimension, by Development area, by Respondent, Current vs Direction, personal vs Team section. The Team score is never a single number; it is whichever Slice the team is looking at, each showing mean, spread and distribution.
